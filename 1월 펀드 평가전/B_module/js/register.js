export default class register {
    ...
    registerLoading() {
        ...
        document.querySelectorAll(".form_box .form-control").forEach(x => {
    ...
    registerCreate() {
        let num = "";
        let randomStr = "QWERTYUIOPASDFGHJKLZXCVBNM";
        while(true) {
            let flag = true;
            num = randomStr.substr(Math.floor(Math.random()*randomStr.length), 1) + Math.random().toString(36).
                substr(2, 3).toUpperCase()+Math.random().toString(10).substr(2, 3).toUpperCase()+Math.random().toString(10).substr(2, 1);
            this.list.forEach(x => {
                flag =  !(x.num == num);
            })
            if(flag) break;
    ...
    registerForm(target) {
        ...
        if(id === "exp") box.querySelector("#word").innerHTML = val.length;

        if(val.trim().length < 1) return this.common.warningMsg(box, "값이 비워져있습니다.");
        else if(id === "found__fund" && val.match(fundNum) === null)
            return this.common.warningMsg(box, "투자자명은 영문, 숫자, 띄어쓰기만 가능합니다.");
